...
export default function AgPoRecoInprPage(): React.ReactElement {
  ...
  const dimensionSelectedObj = useMemo(() => dimensiones.find((x) => x.idDimension === idDimensionSel) ?? null, [dimensiones, idDimensionSel]);
  const unidadSelectedObj = useMemo(() => unidades.find((x) => x.idUnidad === idUnidadSel) ?? null, [unidades, idUnidadSel]);
  const politicaSelectedObj = useMemo(() => politicas.find((x) => x.idPolitica === idPoliticaSel) ?? null, [politicas, idPoliticaSel]);
  ...
  const rcCount = useMemo(() => new Set(rows.map((x) => x.idResultado)).size, [rows]);
  const ipCount = useMemo(() => new Set(rows.map((x) => x.idIntervencion)).size, [rows]);
  const totalRows = useMemo(() => rows.length, [rows]);
  const getIndicadoresCount = (row: AgPoRecoInprMasterDto): number => row.cantidadIndicadores ?? 0;
  ...
  async function loadPoliticas(idPeriodo: number, idDimension: number, idUnidad: number) {
    ...
    try {
      const data = await AgPoRecoInprVistaAction.getPoliticas(idPeriodo, idDimension, idUnidad);
      setPoliticas(data ?? []);
      setIdPoliticaSel((current) => (data?.some((x) => x.idPolitica === current) ? current : data?.[0]?.idPolitica ?? 0));
    } catch (e) {
      console.error(e);
      setPoliticas([]);
      setIdPoliticaSel(0);
  ...
  async function loadTabla(idPeriodo: number, idDimension: number, idUnidad: number, idPolitica: number) {
    ...
    try {
      const data = await AgPoRecoInprVistaAction.getMaster(idPeriodo, idDimension, idUnidad, idPolitica);
      setRows(data ?? []);
      setOpenRowMap({});
      setDetailMap({});
    } catch (e) {
  ...
  async function reloadRowDetail(idAgPoRecoInpr: number) {
    try {
      setDetailMap((prev) => ({ ...prev, [idAgPoRecoInpr]: { loading: true, data: [] } }));
      const data = await AgPoRecoInprVistaAction.getDetail(idAgPoRecoInpr);
      setDetailMap((prev) => ({ ...prev, [idAgPoRecoInpr]: { loading: false, data: data ?? [] } }));
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : "No se pudo recargar el detalle.";
      setDetailMap((prev) => ({ ...prev, [idAgPoRecoInpr]: { loading: false, data: [], error: msg } }));
    }
  }
  ...
  function openIndicadorModal(row: AgPoRecoInprMasterDto, indicador: AgPoRecoInprDetailDto) {
    setIndicadorModal({
      open: true,
  ...
  useEffect(() => {
    if (!loading && idPeriodoSel && idDimensionSel && idUnidadSel && idPoliticaSel) {
    ...
    <Box sx={{ p: 3 }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2}>
        <Stack direction="row" spacing={1} alignItems="center">
          <IconButton onClick={() => navigate(-1)} aria-label="Volver"><ArrowBackRoundedIcon /></IconButton>
          <Box>
            <Typography variant="h5" sx={{ fontWeight: 800 }}>A.G.: Políticas / RC / IP</Typography>
            <Typography variant="body2" color="text.secondary">Vista por Periodo, Dimensión, Unidad Organizacional y Política</Typography>
          </Box>
        </Stack>

        <Stack direction="row" spacing={1} alignItems="center">
          <Chip label={`RC: ${rcCount}`} variant="outlined" />
          <Chip label={`IP: ${ipCount}`} variant="outlined" />
          <Chip label={`Registros: ${totalRows}`} variant="outlined" />
          <Tooltip title="Refrescar" arrow>
            <IconButton onMouseDown={(e) => e.currentTarget.blur()} onClick={onRefresh}><RefreshRoundedIcon /></IconButton>
          </Tooltip>
        </Stack>
      </Stack>
      ...
      <Paper sx={{ mt: 2, p: 2, borderRadius: 3, border: "1px solid", borderColor: "divider", boxShadow: "0 10px 30px rgba(0,0,0,.06)" }}>
        <Box sx={{ width: "100%", mb: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} spacing={2} sx={{ width: "100%", mb: 2 }}>
            <Autocomplete options={periodos} value={periodoSelectedObj} onChange={(_e, newValue) => setIdPeriodoSel(newValue?.idPeriodo ?? 0)} getOptionLabel={(o) => `${o.codigo ?? "—"} - ${o.descripcion ?? "—"}`} isOptionEqualToValue={(o, v) => o.idPeriodo === v.idPeriodo} filterOptions={(options, state) => filterByTexto(options, state.inputValue)} renderInput={(params) => <TextField {...params} label="Periodo" size="small" />} sx={{ flex: 1, ...comboSx }} />
            <Autocomplete options={dimensiones} value={dimensionSelectedObj} onChange={(_e, newValue) => setIdDimensionSel(newValue?.idDimension ?? 0)} getOptionLabel={(o) => `${o.codigo ?? "—"} - ${o.nombre ?? "—"}`} isOptionEqualToValue={(o, v) => o.idDimension === v.idDimension} filterOptions={(options, state) => filterByTexto(options, state.inputValue)} renderInput={(params) => <TextField {...params} label="Dimensión" size="small" />} sx={{ flex: 1, ...comboSx }} />
          </Stack>
        ...
        <Autocomplete
            ...
            onChange={(_e, newValue) => setIdPoliticaSel(newValue?.idPolitica ?? 0)}
        ...
        <TextField value={qSearch} onChange={(e) => setQSearch(e.target.value)} placeholder="Buscar por política, RC o IP..." fullWidth size="small" sx={{ "& .MuiOutlinedInput-root": { borderRadius: 2.5 } }} />
      </Paper>
      ...
      <TableContainer sx={{ mt: 2, borderRadius: 3, border: "1px solid", borderColor: "divider" }}>
        <Table size="small">
          ...
          <TableBody>
            {loadingTabla ? (
              <TableRow><TableCell colSpan={6}><Stack direction="row" spacing={1.5} alignItems="center"><CircularProgress size={18} /><Typography variant="body2">Cargando registros...</Typography></Stack></TableCell></TableRow>
            ) : rowsFiltered.length === 0 ? (
              <TableRow><TableCell colSpan={6}><Alert severity="info" sx={{ borderRadius: 2, width: "100%" }}>No hay registros para la combinación seleccionada.</Alert></TableCell></TableRow>
            ) : (
              rowsFiltered.map((r) => {
                ...
                return (
                  <React.Fragment key={r.idAgPoRecoInpr}>
                    <TableRow hover>
                      <TableCell sx={{ width: 88, verticalAlign: "middle", py: 1.5 }}>
                        <Stack direction="row" spacing={0.75} alignItems="center" justifyContent="center" sx={{ minHeight: 56 }}>
                          <Tooltip title={open ? "Ocultar detalle" : "Ver detalle"} arrow>
                            <IconButton size="small" onMouseDown={(e) => e.currentTarget.blur()} onClick={() => void toggleRowDetail(r)} sx={{ width: 30, height: 30, borderRadius: 2, border: "1px solid", borderColor: "divider", bgcolor: open ? "action.hover" : "transparent", boxShadow: "0 4px 12px rgba(0,0,0,.05)", transition: "all .15s ease", "&:hover": { transform: "translateY(-1px)" } }}>
                              {open ? <KeyboardArrowUpRoundedIcon sx={{ fontSize: 20 }} /> : <KeyboardArrowDownRoundedIcon sx={{ fontSize: 20 }} />}
                            </IconButton>
                          </Tooltip>

                          <Chip size="small" variant="outlined" label={`IND:${indicadoresCount}`} sx={{ height: 20, borderRadius: 2, fontWeight: 900, "& .MuiChip-label": { px: 0.75, fontSize: 11 } }} />
                        </Stack>
                      </TableCell>

                      <TableCell sx={{ fontWeight: 900, verticalAlign: "middle" }}>{safeText(r.codigoRc)}</TableCell>
                      <TableCell sx={{ whiteSpace: "normal", wordBreak: "break-word", overflowWrap: "break-word", verticalAlign: "middle" }}>{safeText(r.descripcionRc)}</TableCell>
                      <TableCell sx={{ fontWeight: 900, verticalAlign: "middle" }}>{safeText(r.codigoIp)}</TableCell>
                      <TableCell sx={{ whiteSpace: "normal", wordBreak: "break-word", overflowWrap: "break-word", verticalAlign: "middle" }}>{safeText(r.descripcionIp)}</TableCell>

                      <TableCell align="right" sx={{ verticalAlign: "middle", width: 150, ...sxStickyActionCell }}>
                        <Stack direction="row" spacing={0.75} justifyContent="flex-end" alignItems="center">
                          <Chip size="small" label={`IND:${indicadoresCount}`} variant="outlined" sx={{ fontWeight: 900, height: 20, "& .MuiChip-label": { px: 0.75, fontSize: 11 } }} />
                          <Tooltip title={open ? "Ocultar detalle" : "Ver detalle"} arrow>
                            <IconButton size="small" onMouseDown={(e) => e.currentTarget.blur()} onClick={() => void toggleRowDetail(r)} sx={{ width: 30, height: 30, borderRadius: 2, border: "1px solid", borderColor: "divider", bgcolor: "rgba(59,130,246,.10)", boxShadow: "0 4px 12px rgba(0,0,0,.05)", transition: "all .15s ease", "&:hover": { transform: "translateY(-1px)", bgcolor: "rgba(59,130,246,.16)" } }}>
                        ...
                        <Collapse in={open} timeout="auto" unmountOnExit>
                          <Box sx={{ p: 2, bgcolor: "background.default" }}>
                            <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2} sx={{ mb: 1 }}>
                              <Stack spacing={0.25}>
                                <Typography variant="subtitle2" sx={{ fontWeight: 900 }}>Detalle Indicadores</Typography>
                                <Typography variant="caption" color="text.secondary">{`RC: ${safeText(r.codigoRc)} — ${safeText(r.descripcionRc)} | IP: ${safeText(r.codigoIp)} — ${safeText(r.descripcionIp)}`}</Typography>
                              </Stack>

                              <Tooltip title="Recargar detalle" arrow>
                                <IconButton size="small" onMouseDown={(e) => e.currentTarget.blur()} onClick={() => void reloadRowDetail(r.idAgPoRecoInpr)} sx={{ borderRadius: 2, border: "1px solid", borderColor: "divider" }}>
                            ...
                            {detail?.loading ? (
                              <Stack direction="row" spacing={1.5} alignItems="center"><CircularProgress size={18} /><Typography variant="body2">Cargando detalle...</Typography></Stack>
                            ) : detail?.error ? (
                              <Alert severity="warning" sx={{ borderRadius: 2 }}>{detail.error}</Alert>
                            ) : !detail?.data || detail.data.length === 0 ? (
                              <Alert severity="info" sx={{ borderRadius: 2 }}>No hay indicadores registrados para este registro.</Alert>
                            ) : (
                              <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: 2 }}>
                                <Table size="small">
                                  <TableHead>
                                    <TableRow>
                                      <TableCell sx={{ fontWeight: 900, width: 180 }}>Código Indicador</TableCell>
                                      <TableCell sx={{ fontWeight: 900 }}>Nombre Indicador</TableCell>
                                      <TableCell align="right" sx={{ fontWeight: 900, width: 120 }}>Acción</TableCell>
                                    </TableRow>
                                  </TableHead>
                                  <TableBody>
                                    {detail.data.map((d) => (
                                      <TableRow key={d.idIndicadorNombre} hover>
                                        <TableCell sx={{ fontWeight: 900 }}>{d.codigoIndicador}</TableCell>
                                        <TableCell sx={{ whiteSpace: "normal", wordBreak: "break-word" }}>{d.nombreIndicador}</TableCell>
                                        <TableCell align="right">
                                          <Tooltip title="Ver indicador" arrow>
                                            <IconButton size="small" onMouseDown={(e) => e.currentTarget.blur()} onClick={() => openIndicadorModal(r, d)} sx={{ width: 30, height: 30, borderRadius: 2, border: "1px solid", borderColor: "divider", bgcolor: "rgba(59,130,246,.10)", "&:hover": { bgcolor: "rgba(59,130,246,.16)" } }}>
