...
// ✨ NUEVO DTO para header completo con OER/AER/OEI/AEI
export type PoiOeiAeiAoHeaderDto = {
  idAei: number;
  codigoAei: string | null;
  enunciadoAei: string | null;
  idOei: number;
  codigoOei: string | null;
  enunciadoOei: string | null;
  idAer: number | null;
  codigoAer: string | null;
  enunciadoAer: string | null;
  idOer: number | null;
  codigoOer: string | null;
  enunciadoOer: string | null;
};
...
export const PoiOeiAeiAoAction = {
  ...
  // ✨ NUEVO: Obtener header completo (OER/AER/OEI/AEI) para modo readonly
  async getHeaderByIdAo(idOeiAeiAo: number): Promise<PoiOeiAeiAoHeaderDto | null> {
    try {
      console.log(`🌐 GET /api/poi/oei-aei-ao/${idOeiAeiAo}/header`);
      const resp = await api.get(`/api/poi/oei-aei-ao/${idOeiAeiAo}/header`);
      console.log("📥 Header recibido:", resp);
      ...
      if (!data) {
        console.warn("⚠️ Header vacío o inválido");
        return null;
      }

      // 🔧 MAPEAR de PascalCase a camelCase
      const rawData = data as Record<string, unknown>;
      const mapped: PoiOeiAeiAoHeaderDto = {
        idAei: (rawData.IdAei ?? rawData.idAei) as number,
        codigoAei: (rawData.CodigoAei ?? rawData.codigoAei) as string | null,
        enunciadoAei: (rawData.EnunciadoAei ?? rawData.enunciadoAei) as string | null,
        idOei: (rawData.IdOei ?? rawData.idOei) as number,
        codigoOei: (rawData.CodigoOei ?? rawData.codigoOei) as string | null,
        enunciadoOei: (rawData.EnunciadoOei ?? rawData.enunciadoOei) as string | null,
        idAer: (rawData.IdAer ?? rawData.idAer) as number | null,
        codigoAer: (rawData.CodigoAer ?? rawData.codigoAer) as string | null,
        enunciadoAer: (rawData.EnunciadoAer ?? rawData.enunciadoAer) as string | null,
        idOer: (rawData.IdOer ?? rawData.idOer) as number | null,
        codigoOer: (rawData.CodigoOer ?? rawData.codigoOer) as string | null,
        enunciadoOer: (rawData.EnunciadoOer ?? rawData.enunciadoOer) as string | null,
      };

      console.log("🔄 Header mapeado:", mapped);
      return mapped;

    } catch (error) {
      console.error("❌ Error en getHeaderByIdAo:", error);
      return null;
    }
  },
};
