...
export default function DashboardAgPage(): React.ReactElement {
  ...
  async function loadCombos() {
    ...
    try {
      const [periodosResp, aniosResp] = await Promise.all([
        DashboardCatalogoAction.getPeriodos("AG"),
  ...
  function openIndicadorDrawer(item: DashboardAgJerarquiaDto) {
    setDrawerState({
      open: true,
      instrumento: "AG",
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
            Políticas, resultados concertados, intervenciones e indicadores
          </Typography>
        </Box>
        ...
        <>
          <Grid container spacing={2.2} sx={{ mt: 0.5 }}>
            <Grid item xs={12} md={6} xl={2.4}>
              <KpiCard title="Políticas" value={data.kpis.totalPoliticas} subtitle="Políticas estratégicas" />
            </Grid>
            <Grid item xs={12} md={6} xl={2.4}>
              <KpiCard title="Resultados" value={data.kpis.totalResultados} subtitle="Resultados concertados" />
            </Grid>
            <Grid item xs={12} md={6} xl={2.4}>
              <KpiCard title="Intervenciones" value={data.kpis.totalIntervenciones} subtitle="Intervenciones prioritarias" />
            </Grid>
            <Grid item xs={12} md={6} xl={2.4}>
              <KpiCard title="Indicadores" value={data.kpis.totalIndicadores} subtitle="Indicadores vinculados" />
            </Grid>
            <Grid item xs={12} md={6} xl={2.4}>
              <KpiCard title="Avance Promedio" value={formatPercent(data.kpis.avancePromedio)} subtitle="Promedio consolidado AG" />
            </Grid>
          </Grid>

          <Paper elevation={0} sx={{ mt: 2.2, p: 2, borderRadius: 3, border: "1px solid", borderColor: "divider", boxShadow: "0 10px 24px rgba(0,0,0,.05)" }}>
            <Typography sx={{ fontWeight: 900, mb: 2 }}>Jerarquía Política / Resultado / Intervención</Typography>

            <Box sx={{ height: 320 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={jerarquiaData}>
                  <CartesianGrid stroke={DASHBOARD_COLORS.grid} strokeDasharray="3 3" />
                  <XAxis dataKey="codigoIntervencion" tick={{ fill: DASHBOARD_COLORS.text, fontSize: 12 }} />
                  <YAxis tick={{ fill: DASHBOARD_COLORS.text, fontSize: 12 }} />
                  <RechartsTooltip formatter={(v: number) => formatPercent(v)} />
                  <Bar dataKey="avancePromedio" name="Avance Promedio" fill={DASHBOARD_COLORS.success} radius={[8, 8, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </Box>
            ...
            <Stack spacing={1.1} sx={{ mt: 2 }}>
              {jerarquiaData.map((item) => (
                <Paper key={item.idAgPoRecoInpr} variant="outlined" sx={{ p: 1.4, borderRadius: 2 }}>
                  <Stack direction={{ xs: "column", md: "row" }} spacing={1} justifyContent="space-between" alignItems={{ md: "flex-start" }}>
                    <Box sx={{ minWidth: 0, flex: 1 }}>
                      ...
                      <Typography sx={{ mt: 0.35, fontSize: 13, color: "text.secondary" }}>
                        {item.codigoResultado} - {item.descripcionResultado}
                      </Typography>
                      <Typography sx={{ mt: 0.25, fontSize: 13, color: "text.secondary" }}>
                        {item.codigoIntervencion} - {item.descripcionIntervencion}
                      </Typography>
          ...
          <Paper elevation={0} sx={{ mt: 2.2, p: 2, borderRadius: 3, border: "1px solid", borderColor: "divider", boxShadow: "0 10px 24px rgba(0,0,0,.05)" }}>
            <Typography sx={{ fontWeight: 900, mb: 2 }}>Tendencia AG</Typography>
            ...
            <Box sx={{ height: 320 }}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={tendenciaData}>
                  ...
                  <YAxis tick={{ fill: DASHBOARD_COLORS.text, fontSize: 12 }} />
                  <RechartsTooltip formatter={(v: number) => formatPercent(v)} />
                  <Line type="monotone" dataKey="valor" name="Avance" stroke={DASHBOARD_COLORS.success} strokeWidth={3} dot={{ r: 4, fill: DASHBOARD_COLORS.success }} activeDot={{ r: 6 }} />
                </LineChart>
              </ResponsiveContainer>
            </Box>
          </Paper>
